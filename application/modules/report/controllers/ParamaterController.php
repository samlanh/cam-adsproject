...
// 					"category_id"=>-1,
	...
	public function rptIncomeRentSaleAction(){
		...
		$this->view->list_end_date = $search;
		$db = new Report_Model_DbTable_DbParamater();
		$this->view->rentincome = $db->getRentIcome($search);
		$this->view->saleincome = $db->getSaleIcome($search);
	...
	public function rptBookExpireAction(){
		if($this->getRequest()->isPost()){
			$search=$this->getRequest()->getPost();
		}
		else{
		...
		$this->view->search = $search;
		$db  = new Report_Model_DbTable_DbParamater();
		$this->view->row = $db->getBookingExpire($search);
