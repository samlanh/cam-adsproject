  ...
  (SELECT
   ...
   LIMIT 1) AS `property_street`
    ...
    function getIncomeCategory($search){
		...
		";
    	$order =" GROUP BY ic.`category_id` ORDER BY ic.`category_id` ASC";
    	$where="";
    ...
    function getBuyland($search=null){
    	$db = $this->getAdapter();
    	$sql="SELECT *,
    	(SELECT p.type_nameen FROM `ln_properties_type` AS p WHERE p.id = bl.`land_type` LIMIT 1) AS land_type_name,
    	(SELECT v.name_kh FROM `ln_view` AS v WHERE v.type=11 AND v.key_code = bl.sale_sex LIMIT 1) AS sale_sex_name,
    	(SELECT v.name_kh FROM `ln_view` AS v WHERE v.type=11 AND v.key_code = bl.buyer_sex LIMIT 1) AS buyer_sex_name
    	...
    	if (empty($search['show_all'])){
    		...
    		if($search['property_type_search']>-1){
    			$where.= " AND bl.`land_type` = ".$search['property_type_search'];
    		}
    	}
    	$order=' ORDER BY bl.id DESC';
    	return $db->fetchAll($sql.$where.$order);
    }
    public function getRentIcome($search=null){
    	$db = $this->getAdapter();
    	$sql="SELECT
    	r.`id`,r.`rent_no`,
    	...
    	FROM `ln_rent` AS r  WHERE r.`status`=1";
    	$where='';
    ...
    public function getSaleIcome($search=null){
    	$db = $this->getAdapter();
    	...
    	FROM `ln_sale_property` AS s WHERE s.`status` = 1";
    	$where='';
    	$from_date =(empty($search['start_date']))? '1': "s.`buy_date` >= '".$search['start_date']." 00:00:00'";
    	$to_date = (empty($search['end_date']))? '1': "s.`buy_date` <= '".$search['end_date']." 23:59:59'";
    	if (empty($search['show_all'])){
    ...
    public function getBookingExpire($search=null){
    	$db =$this->getAdapter();
    	$sql= 'SELECT *,
    	(SELECT `ln_project`.`project_name`   FROM `ln_project`  WHERE (`ln_project`.`br_id` = `s`.`branch_id`) LIMIT 1) AS `project_name` ,
    	(SELECT  `ln_view`.`name_kh`  FROM `ln_view`   WHERE (`ln_view`.`key_code` = `s`.`payment_id` AND `ln_view`.`type`=25)
    	LIMIT 1) AS `payment_type` ,
    	`p`.`land_code`       AS `land_code`,
    	`p`.`land_address`    AS `land_address`,
    	`p`.`property_type`   AS `property_type`,
    	(SELECT     `ln_properties_type`.`type_nameen`   FROM `ln_properties_type`   WHERE (`ln_properties_type`.`id` = `p`.`property_type`)
    	...
    	`ln_properties` AS p,
    	`ln_client` AS `c`
    	WHERE p.`id`=s.`house_id` AND c.`client_id` = s.`client_id` AND  s.`payment_id`=1 AND s.`is_cancel`=0 ';
    	$where='';
    	$where = ' AND DATE(s.`end_line`)< DATE("'.$search['end_date'].'")';
    	if(!empty($search['adv_search'])){
    		$s_where = array();
    		$s_search = addslashes(trim($search['adv_search']));
    		$s_where[] = " `p`.`land_code`  LIKE '%{$s_search}%'";
