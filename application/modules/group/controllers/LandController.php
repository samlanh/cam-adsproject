...
// 						'streetlist'=>$formdata['streetlist'],
			...
			$collumns = array("BRANCH_NAME","PROPERTY_CODE","HEAD_TITLE_NO","PROPERTY_TYPE","PRICE","WIDTH","HEIGHT","SIZE","DATE","BY_USER","STATUS");
			$link=array(
			...
			$this->view->list=$list->getCheckList(3, $collumns, $rs_rows,array('branch_name'=>$link1,'land_code'=>$link,'land_address'=>$link,'pro_type'=>$link,
					'price'=>$link));
		}catch (Exception $e){
	...
	function deleteAction(){
		...
		if (!empty($id)){
			try{
				$check = $db->CheckDeleteLand($id);
				if (!empty($check)){
					$db->deleteLand($id);
					Application_Form_FrmMessage::Sucessfull('DELETE_SUCCESS',"/group/land");
				}else{
					Application_Form_FrmMessage::Sucessfull('ដីនេះបានដាក់លក់ឬក៏ជួលរួចហើយមិនអាចលុបបានទេ',"/group/land");
				}
			}catch (Exception $e){
