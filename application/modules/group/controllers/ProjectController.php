...
class Group_ProjectController extends Zend_Controller_Action {
      ...
      try{
    	...
    	$search=$this->getRequest()->getPost();
           ...
           $glClass = new Application_Model_GlobalClass();
			...
			$this->view->list=$list->getCheckList(3, $collumns, $rs_rowshow,array('project_name'=>$link,'project_type'=>$link));
		}catch (Exception $e){
	...
	function deleteAction(){
		...
		if (!empty($id)){
			try{
				$db->deleteProject($id);
				Application_Form_FrmMessage::Sucessfull('DELETE_SUCCESS',"/group/project");
			}catch (Exception $e){
