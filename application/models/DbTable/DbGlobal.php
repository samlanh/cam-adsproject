...
//    }
   ...
   public function getNewClientId(){
   	...
   	$db = $this->getAdapter();
   	$sql=" SELECT client_id ,client_number FROM $this->_name WHERE type=1  ORDER BY client_id DESC LIMIT 1 ";
   	$acc_no = $db->fetchOne($sql);
   	$new_acc_no= (int)$acc_no+1;
   	$acc_no= strlen((int)$acc_no+1);
   	$pre = "";
...
//   return $this->getAdapter()->fetchAll($sql);	
  ...
  public function getAllLandInfo($branch_id=null,$option=null,$action=null){
  	   $db = $this->getAdapter();
  	   $sql="SELECT `id`,CONCAT(`land_address`) AS name FROM `ln_properties` WHERE `land_address`!='' ";//just concate
  	   $request=Zend_Controller_Front::getInstance()->getRequest();
  	   if($action==null){
  	   	$sql.=" AND status=1  AND `is_lock`=0  ";
  	   }else{
...
//   	echo $sql;
  ...
  function getAllClientNumber($branch_id=null){
  	...
  	FROM `ln_client` AS c WHERE c.`name_kh`!='' AND c.client_number !='' AND c.status=1 AND c.type=1 " ;
//   	if($branch_id!=null){
//   		$sql.=" AND c.`branch_id`= $branch_id ";
//   	}
  ...
  function getAllClient($branch_id=null){
  	...
  	FROM `ln_client` AS c WHERE c.`name_kh`!='' AND c.status=1 AND c.type=1" ;
  	if($branch_id!=null){
  		$sql.=" AND c.`branch_id`= $branch_id ";
  	}
  	 $sql.=" ORDER BY c.`client_id` DESC";
...
$sql = " SELECT g.co_id,m.client_id  FROM  `ln_loan_member` AS m , `ln_loan_group` AS g
  ...
  public function getNewClientIdByBranch($branch_id){// by vandy get new client no by branch
  	$this->_name='ln_client';
  	$db = $this->getAdapter();
  	$sql=" SELECT count(client_id)  FROM $this->_name WHERE type=1 AND  branch_id = $branch_id LIMIT 1 ";
  	$acc_no = $db->fetchOne($sql);
  ...
  public function getPropertyTypeForsearch(){
  	...
  	$sql="SELECT t.`id`,t.`type_nameen` AS `name` FROM `ln_properties_type` AS t WHERE t.`status`=1";
  	$rows =  $db->fetchAll($sql);
  	$options=array(''=>"ជ្រើសរើសប្រភេទដី");
  	if(!empty($rows))foreach($rows AS $row){
  		$options[$row['id']]=$row['name'];//($row['displayby']==1)?$row['name_kh']:$row['name_en'];
  	}
  	return $options;
  ...
  public function getConstractor(){
  	$db = $this->getAdapter();
  	$sql = "SELECT c.`id`,c.`namekh` AS `name` FROM `ln_contract_person` AS c WHERE c.`status`=1";
  	return $db->fetchAll($sql);
  }
