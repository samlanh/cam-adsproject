...
jQuery(document).ready(function($){
  ...
  if (jQuery('#mylist li').length == 0)	{
	...
	jQuery("#catID").on('change',function() {
		...
		jQuery('.extra-fields-container').append('<div class="text-center spinner-contain"><i class="fa fa-spinner fa-spin fa-3x"></i></div>');
		jQuery('.c-custom-fields').remove();
         jQuery.ajax({
             type: 'POST',
             dataType: 'html',
             url: ajaxurl,
             data: { action: 'category_field_ajax', catid: $val },
             success: function(response){
				jQuery('.extra-fields-container').append(response);
				jQuery('.extra-fields-container .spinner-contain').remove();
             }
         });
